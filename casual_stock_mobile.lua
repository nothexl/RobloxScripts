...
-- Окно
local Window = Library.CreateLib("Casual Stock [СКРИПТ]", "DarkTheme")

-- Вкладки
local MainTab = Window:NewTab("Функции")
local VisualsTab = Window:NewTab("Магазины")
local BotTab = Window:NewTab("Бот")

-- Вкладка магазионв
_G.RefreshRate = 0.5
local ShopSettingsSection = VisualsTab:NewSection("Настройки")
local RefreshRateSlider = ShopSettingsSection:NewSlider("Время обновления", "", 2, 1,
    function(Value)
        _G.RefreshRate = Value
    end
)

local ColorSection = VisualsTab:NewSection("Цвета")

local OutlineColorPicker = ColorSection:NewColorPicker("Обводка", "", Color3.fromRGB(255,255,255),
    function(Value)
...
local FillColorPicker = ColorSection:NewColorPicker("Заливка", "", Color3.fromRGB(255,0,0),
    function(Value)
        for i, v in pairs(workspace.World.Debris:FindFirstChild("Stores"):GetDescendants()) do
            if v:FindFirstChild("Highlight") then
                local Highlight = v:FindFirstChild("Highlight")
...
local TransparencySlider = ColorSection:NewSlider("Прозрачность", "", 0.6, 0,
    function(Value)
        for i, v in pairs(workspace.World.Debris:FindFirstChild("Stores"):GetDescendants()) do
            if v:FindFirstChild("Highlight") then
                local Highlight = v:FindFirstChild("Highlight")
...
local ShopSection = VisualsTab:NewSection("Магазины")

_G.SportMaster = false
local SportmasterToggle = ShopSection:NewToggle("Спортмастер", "",
    function(Value)
        if Value then
            _G.SportMaster = true
...
_G.Zara = false
local ZaraToggle = ShopSection:NewToggle("Zara", "",
    function(Value)
        if Value then
            _G.Zara = true
...
_G.Bape = false
local BapeToggle = ShopSection:NewToggle("Bape", "",
    function(Value)
        if Value then
            _G.Bape = true
...
local StoneIslandToggle = ShopSection:NewToggle("Stone Island", "",
    function(Value)
        if Value then
            _G.StoneIsland = true
...
_G.Balenciaga = false
local BalenciagaToggle = ShopSection:NewToggle("Balenciaga", "", 
    function(Value)
        if Value then
            _G.Balenciaga = true
...
local ChromeHeartsToggle = ShopSection:NewToggle("Chrome Hearts", "",
    function(Value)
        if Value then
...
local LouisVuittonToggle = ShopSection:NewToggle("Louis Vuitton", "",
    function(Value)
        if Value then
...
_G.Gucci = false
local GucciToggle = ShopSection:NewToggle("Gucci", "",
    function(Value)
        if Value then
            _G.Gucci = true
...
-- Вкладка функций
local MainSection = MainTab:NewSection("Автоматические")

_G.Grab = false
local GrabToggle = MainSection:NewToggle("Подбор", "",
    function(Value)
        if Value then
            _G.Grab = true
...
_G.Buy = false
local BuyToggle = MainSection:NewToggle("Покупка", "",
    function(Value)
        if Value then
            _G.Buy = true
...
_G.Sell = false
local SellToggle = MainSection:NewToggle("Продажа", "",
    function(Value)
        if Value then
            _G.Sell = true
...
_G.Distance = false
local DistanceToggle = MainSection:NewToggle("Максимальная дистанция", "",
    function(Value)
        if Value then
            _G.Distance = true
...
local PlayerSection = MainTab:NewSection("Персонаж")

local WalkSlider = PlayerSection:NewSlider("Скорость", "", 38, 16,
    function(Value)
 ...
 _G.Noclip = false
local NoclipToggle = PlayerSection:NewToggle("NoClip", "",
    function(Value)
...
local BotSettingSection = BotTab:NewSection("Настройки")

_G.PauseRate = 1
 local TeleportSlider = BotSettingSection:NewSlider("Время паузы", "", 3, 0.5,
   function(Value)
        _G.PauseRate = Value
   end
)

local MainBotSection = BotTab:NewSection("Основное")

_G.IsLegitMode = true
local BotMode = MainBotSection:NewDropdown("Режим", "", {"Ходьба", "Телепорт"},
    function(Option)
...
_G.EnableBot = false
local EnableBotToggle = MainBotSection:NewToggle("Включить", "",
    function(Value)
